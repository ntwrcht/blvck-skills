...
- Any Thai prose deliverable a paragraph or longer — blog post, landing page,
  doc page, Thai README, email, announcement, a section of a larger document.
- Translating English source material into Thai.
- Editing, rewriting, or reviewing Thai prose that already exists — including
  prose an agent drafted earlier in the session.

## When Not to Use

- Single words or phrases: button labels, menu items, short UI strings, code
  identifiers, code comments. The frames govern discourse structure and have
...
The machine-sounding Thai the frames below fight is largely an artifact of
English-centric models writing Thai; a Thai-pretrained model carries the native
distribution in its weights rather than translating into it. Where such a model
is reachable, the strongest result comes from drafting with it and using these
frames as the audit layer. That claim rests on limited evidence and is not
settled. Absent a native-model draft, this skill both drafts and audits, exactly
as the rest of this file describes — it stands alone.
...
<!-- Native side lifted from a somkiat.cc post on bun testing;
...
task prompt. Scope `register.md`, `examples.md`, and `exemplars.md` to the
active register when it is known — loading all eight in full is expensive.

The rules descend from published Thai writing (Blognone long-form, Somkiat
Puisungnoen, bank explainer columns, The Standard, The 101 World, Bookscape
translations) and from scholarly work on Thai discourse — Iwasaki &
Ingkaphirom, Smyth, Prasithrathsint, Takahashi, Olsson, the Thai Discourse
...
author reads it before it ships.
