...
A run costs roughly $0.13 per case per run, baseline arm included; the default is 3 runs. Triggering varies run to run, so use `--runs 1` while iterating and the default for the final pass.
