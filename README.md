...
| Skill Name | Description |
| :--- | :--- |
| [**Agent Smith**](skills/productivity/agent-smith/SKILL.md) | Designs Claude Code subagents as domain experts: an expert role, preloaded skills, an explicit tool and model budget, sibling boundaries, and a delegation test. Use when creating, writing, or reviewing a subagent, agent persona, or agent definition file, or planning a roster of agents. |
...
| [**Skill Smith**](skills/productivity/skill-smith/SKILL.md) | Crafts reusable agent skills from confirmed use cases, with invocation design, progressive disclosure, bundled resources, and evals that prove the skill triggers and delivers the right output. Use when the user asks to create, write, review, improve, or test a skill, upgrade an existing SKILL.md, check whether a skill triggers, or package skill references, scripts, or examples. |
...
| [**Write a Story**](skills/productivity/write-a-story/SKILL.md) | Drafts, splits, rewrites, and reviews individual backlog items — stories, tasks, bugs — with testable acceptance criteria and approved Jira payloads. Use when writing a story, splitting a feature into issues, adding acceptance criteria, checking sprint readiness, or breaking work into tickets. |
...
| [**Wait, What**](skills/misc/wait-what/SKILL.md) | Re-pitches the message that just failed to land, adding the missing context and dropping the jargon. Use when a reply lost the reader through unexplained jargon, a skipped step, or missing context; the user runs it with /wait-what. |
